...
def Dataset_KFold_Sample(dataset, input_dir, session, target_id, k, trial, fold, shuffle=True):
...
def Dataset_KFold_Trial(dataset, input_dir, session, target_id, k, trial, fold,  shuffle=True):
    global shuffle_indices
    data_dir = None
    if dataset == 'SEED' or dataset == 'SEEDIV':
        # SEED
...
def Dataset_KFold_USTrial(dataset, input_dir, session, target_id, k, trial, fold):
    data_dir = None
    if dataset == 'SEED' or dataset == 'SEEDIV':
        # SEED
        data_dir = os.path.join(input_dir, f'Session{session}')
    ...
    for session in range(1, 4):
        for i in range(1, subjects+1):
            for fold in range(kfold):
                train_dataset, test_dataset = Dataset_KFold_Trial(dataset, input_dir, session, i, kfold, trial, fold)
                # train_dataset, test_dataset = Dataset_KFold_Sample(dataset, input_dir, session, i, kfold, trial, fold)
